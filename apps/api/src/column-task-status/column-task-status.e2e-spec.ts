...
import {
  columnTaskStatus,
  notValidColumnTaskStatus,
  notValidColumnTaskStatusErrors,
  ColumnTaskStatusMock,
} from './mock';

describe('ColumnTaskStatusModule (e2e)', () => {
  const url = '/column-task-status/';

  const test = new BaseTest(new TestApp(), url, {
    post: {
      body: columnTaskStatus,
      notValidBody: notValidColumnTaskStatus,
      errorsList: notValidColumnTaskStatusErrors,
    },
    patch: {
      body: new ColumnTaskStatusMock(),
    },
  });

  beforeAll(async () => {
    await test.app.setup([ColumnTaskStatusModule]);
  });
